...
import { causeData, cancerNames } from "../data/interest-data";
import _ from "lodash";
import styled from "styled-components";

const greatestInterest = _.maxBy(causeData, "value");
...
const CauseChart = () => {
  ...
  const Results = () => (
    <TopNames id="results">{cancerNames.map((index) => {
      return (
        <p>{index}</p>
      )
    }
    ...
    <div className="container">
      <div className="reverse-fave-container">
        ...
        <button title="Show Team Members" onMouseEnter={onClick} onMouseLeave={onClick}>
          ...
          <h4>is your team's favorite cause</h4>
        </div>
      </div>
