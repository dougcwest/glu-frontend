...
import { movieData, comedyNames } from "../data/interest-data";
import _ from "lodash";
import styled from "styled-components";

const greatestInterest = _.maxBy(movieData, "value");
...
export default function MovieChart() {
  ...
  const Results = () => (
    <TopNames id="results">{comedyNames.map((index) => {
      return (
        <p>{index}</p>
      )
    }
    ...
    <div className="reverse-container">
      <div className="fave-container">
        ...
        <button title="Show Team Members" onMouseEnter={onClick} onMouseLeave={onClick}>
          ...
          <h4>is your team's favorite genre</h4>
        </div>
      </div>
